...
import { DIRECTORY_MODE, REGULAR_FILE_MODE } from "./constants";
import { TrakRepository } from "./repository";
import { TrakDatabase } from "./database";
import { Author, TrakBlob, TrakCommit, TrakObject, TrakTree } from "./objects";
...
export async function commit(message: string, author: Author) {
    const repo = await TrakRepository.repoFind();
    if (!repo)
        return;
    ...
    const indexContent = await TrakDatabase.loadIndex(repo);
    ...
    if (parentCommit) {
        const parentCommitData = (await TrakDatabase.readObject(repo, parentCommit)) as TrakCommit;
    ...
    const commitHash = await TrakDatabase.writeObject(commit, repo);;
...
export async function log() {
    ...
    while (commitHash && count < maxCount) {
        const commit = (await TrakDatabase.readObject(repo, commitHash)) as TrakCommit;
...
export async function checkout(branchName?: string, createBranch?: boolean) {
    ...
    try {
        ...
        if (previousCommitHash) {
            const previousCommit = (await TrakDatabase.readObject(repo, previousCommitHash)) as TrakCommit;
...
async function _getFilesFromTree(repo: TrakRepository, treeHash: string, prefix: string = ""): Promise<Set<string>> {
    ...
    try {
        const treeObject = (await TrakDatabase.readObject(repo, treeHash)) as TrakTree;
...
async function _restoreWorkingDirectory(repo: TrakRepository, branch: string, filesToClear: Set<string>) {
    ...
    const targetCommitObject = (await TrakDatabase.readObject(repo, targetCommitHash)) as TrakCommit;
...
async function _restoreTree(repo: TrakRepository, treeHash: string, path: string) {
    const treeObject = (await TrakDatabase.readObject(repo, treeHash)) as TrakTree;
    const tree = TrakTree.deserialize(treeObject.content);
    ...
    for (const { mode, name, oid } of tree.entries) {
        ...
        if (mode.startsWith("100")) {
            const blobObject = (await TrakDatabase.readObject(repo, oid)) as TrakBlob;
...
async function _createTreeFromIndex(repo: TrakRepository, indexEntries: Record<string, string>): Promise<string> {
    ...
    if (Object.keys(indexEntries).length === 0) {
        return await TrakDatabase.writeObject(new TrakTree(), repo);
    }
...
async function _createTreeRecursive(repo: TrakRepository, dirTree: DirTree): Promise<string> {
    ...
    for (const [name, content] of Object.entries(dirTree)) {
        if (typeof content === 'string') {
            ...
            tree.addEntry({ mode: REGULAR_FILE_MODE, name, oid: content });
        } else if (typeof content === 'object') {
            const subtreeHash = await _createTreeRecursive(repo, content);
            tree.addEntry({ mode: DIRECTORY_MODE, name, oid: subtreeHash });
        }
    }

    return await TrakDatabase.writeObject(tree, repo);
}
...
async function _addFile(filePath: string, repo: TrakRepository) {
    ...
    const blobHash = await TrakDatabase.writeObject(new TrakBlob(fileData), repo);
    // Load index file json contents
    const indexJSON = await TrakDatabase.loadIndex(repo);
    // Map blob hash to file path: [path] -> hash
    indexJSON[relative(repo.workTree, filePath)] = blobHash;
    // Save map to index file
    await TrakDatabase.saveIndex(repo, indexJSON);
}
    ...
    while (stack.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (stats.isDirectory()) {
                ...
                const blobHash = await TrakDatabase.writeObject(new TrakBlob(fileData), repo);
                // Update index map of blob hash to file path: [path] -> hash
